...
def create_mesh2(skel,outline):
    ...
    for i in range(len(skel)):
        try:
            ...
            if len(ip1)>0 and len(ip2)>0: #making sure both are not none
                lines.append([ip1,ip2])
                print "adding",ip1,ip2
        except:
            print "did not draw a line for",i
    ...
    for i in range(len(lines)-1):
        print "getting region between",lines[i],lines[i+1]
...
def get_skeleton_from_outline_with_extension(outline):
    ...
    try:
        ...
        line = shapely.geometry.LineString([mid_a,[mid_a[0]-d[0]*20,mid_a[1]-d[1]*20]])
        ip_last = get_polygon_line_intersection(poly,line)
        print "ip_last",len(ip_last),ip_last
        if len(ip_last)==0:
            ip_last=-1
        if ip_last == None:
            ip_last=-1
    ...
    print "edges_x3",edges_x3
    skel=np.zeros([len(edges_x3)+num_extra,2])
    print skel.shape,len(edges_x3)
    offset=0
    if ip_first != -1:
        skel[0,:]=ip_first
        offset=1
    for i in range(len(edges_x3)):
        skel[i+offset,:]=edges_x3[i]
    if ip_last != -1 : 
        skel[-1,:]=ip_last
    print "before",skel
    skel_line = shapely.geometry.LineString(skel)
